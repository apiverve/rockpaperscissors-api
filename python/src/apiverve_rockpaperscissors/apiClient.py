...
class RockpaperscissorsAPIClientError(Exception):
...
class ValidationError(RockpaperscissorsAPIClientError):
...
class RockpaperscissorsAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"choice": {"type": "string", "required": False, "default": "random"}, "rounds": {"type": "integer", "required": False, "min": 1, "max": 100, "default": 1}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the RockpaperscissorsAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises RockpaperscissorsAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/rockpaperscissors'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise RockpaperscissorsAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[RockpaperscissorsAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns RockPaperScissorsData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises RockpaperscissorsAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise RockpaperscissorsAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise RockpaperscissorsAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise RockpaperscissorsAPIClientError(f"Request failed: {str(e)}")
